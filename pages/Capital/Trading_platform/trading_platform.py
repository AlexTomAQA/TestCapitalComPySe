...
class TradingPlatform(BasePage):
    ...
    def should_be_trading_platform_page_v4(self, d, test_link, tpd, tpi, trade_instrument):
        ...
        """
        allure.step(f"{datetime.now()}   Checking that the trading platform page has opened - ver 4")
    ...
    def should_be_platform_live_mode(self, d, test_link):
        ...
        allure.step(f"{datetime.now()}   Check if the trading platform opened in LIVE mode")

        print(f"{datetime.now()}   3.2. Checking that the Trading platform opened in LIVE mode =>")
        if not self.element_is_visible(TopBarLocators.MODE_LIVE, 10):
            # проверка бага для ретеста
            print(f'\nBug: {self.bid}')
